...
var Formula = {
  ...
  initValue: function () {
    ...
    if (img && img.getAttribute('data-formula-image') !== null) {
      var formulaConfig = editor.getOpt('formulaConfig');
      var pcs = formulaConfig.imageUrlTemplate.split('{}');
      var regex = '^' + preg_quote(pcs[0]) + '(.*?)' + preg_quote(pcs[1]) + '$';
      var matches = img.getAttribute('src').match(new RegExp(regex));
      if (matches[1]) {
        $('#editor').val(decodeURIComponent(matches[1]));
        Formula.render();
      }
    }
  },
